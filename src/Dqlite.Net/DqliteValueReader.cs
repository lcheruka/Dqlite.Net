...
{
    internal abstract class DqliteValueReader
    {
        public abstract int FieldCount { get; }
        protected abstract DqliteTypes GetDqliteType(int ordinal);

        public virtual bool IsDBNull(int ordinal)
            => GetDqliteType(ordinal) == DqliteTypes.Null;

        public virtual bool GetBoolean(int ordinal)
            => IsDBNull(ordinal)
                ? throw new InvalidOperationException(GetOnNullErrorMsg(ordinal))
                : GetBooleanCore(ordinal);

        protected abstract bool GetBooleanCore(int ordinal);
        ...
        {
            var sqliteType = GetDqliteType(ordinal);
            if (sqliteType == DqliteTypes.Text)
            {
